...
import { XCircle, HelpCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
function DeclinedLeadsPageContent() {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        ...
        console.error("Failed to load declined leads:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  finalParams.set("status", "declined");
    ...
    <div className="p-6 sm:p-8 w-full space-y-8 animate-fade-in text-slate-600">
      ...
      <div>
        <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-slate-800 flex items-center">
          ...
          <span>Declined Leads</span>
        </h1>
        <p className="text-sm text-slate-505 mt-2 font-medium">
          Prospects that have been declined or marked as not interested.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-16 text-center max-w-lg mx-auto flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">No Declined Leads</h3>
          <p className="text-sm text-slate-500 mt-2 leading-relaxed">
            There are no declined leads yet. Browse the database and select prospects to decline.
          </p>
          <div className="mt-6">
            <Link
              href="/leads"
...
export default function DeclinedLeadsPage() {
  return (
    <Suspense
      fallback={
        <div className="p-6 w-full space-y-6 text-slate-500">
          Loading declined leads...
        </div>
      }
    >
      <DeclinedLeadsPageContent />
    </Suspense>
  );
}
