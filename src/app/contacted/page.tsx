...
import { PhoneCall, HelpCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
function ContactedLeadsPageContent() {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        ...
        console.error("Failed to load contacted leads:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  finalParams.set("status", "contacted");
    ...
    <div className="p-4 sm:p-8 w-full space-y-8 animate-fade-in text-slate-600">
      ...
      <div>
        <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-slate-800 flex items-center">
          <PhoneCall className="mr-3 text-emerald-500" size={28} />
          <span>Contacted Leads</span>
        </h1>
        <p className="text-sm text-slate-500 mt-2 font-medium">
          Prospects that have been contacted by your sales reps.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-16 text-center max-w-lg mx-auto flex flex-col items-center justify-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">No Contacted Leads</h3>
          <p className="text-sm text-slate-500 mt-2 leading-relaxed">
            You haven&apos;t contacted any leads yet. Go to the All Leads page and mark some leads as contacted.
          </p>
          <div className="mt-6">
            <Link
              href="/leads"
